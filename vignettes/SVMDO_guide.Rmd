...
# Dataset Preparation
RNA-Seq cancer transcriptome and clinical datasets must be prepared before applying them in SVMDO. The expected forms of input datasets are indicated in Table-1 and Table-2 for transcriptome and clinical datasets respectively
...
table_prep<-data.frame(id,days_to_death,vital_status)
```
`r knitr::kable(table_prep,"pipe",col.name=c("id","days_to_death","vital_status"),align = "cccc",caption="Table-2 Structure of Clinical Dataset")`
...
If there is not any requirement for **survival analysis**, preparation of **clinical dataset** and involvement of **id** column in the transcriptome dataset are optional.
...
6. By clicking on **DEG Analysis** button you further apply differential expression analysis. Labels of tissue_type column in dataset must contain “Nor” and “Tum” for determining normal/tumour (or tumor) samples. A meesage window saying process completed will appear if there is not any problem.
8. When the process is completed, a message window saying **Differential Expression Analysis is Completed** will appear.
...
11. To apply disease ontology-based gene filtration, click on **DO Analysis** button. A meesage window saying **process completed** will appear if there is not any problem.
12. To further apply the following feature selection and classification processes, click on the **Classification** button. A meesage window saying **process completed** will appear if there is not any problem.
15. Acquired discriminative gene set can be further used for survival analysis to detect individual prognostic genes. To apply this process, use the file detection in **Choose Clinical Data** section for searching clinical data about patient survival followed by clicking on **Survival Analysis** button. A meesage window saying **process completed** will appear if there is not any problem.
...
When test datasets are used, predefined input size (n=25) is used. 
1. First of all, select the dataset to be used in test analysis by using radio buttons. There are two datasets consisting of simplified forms of TCGA-COAD (COAD) and TCGA-LUSC (LUSC) with 550 genes.
...
3. To apply survival analysis on the acquired discriminative gene sets, click on **Survival Analysis** button. Clinical dataset for the selected dataset is automatically loaded into the GUI. A meesage window saying **process completed** will appear if there is not any problem.
...
* **prognostic_analysis:** A table of prognostic evaluation of discriminative gene set
* Kaplan Meier plots of individual genes from the final form of discriminative gene set
